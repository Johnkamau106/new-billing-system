...
const HotspotClients = () => {
  ...
  useEffect(() => {
    ...
    fetch('/api/clients')
      ...
      .then(data => {
        setClients(data);
        setFilteredClients(data.filter(client => client.type === 'hotspot'));
      })
  ...
  const handleAddClient = (newClientData) => {
    ...
    setFilteredClients(newClients.filter(client => client.type === 'hotspot'));
  ...
  return (
    <div className="clients-list-page">
      <div className="clients-list-page__header">
        <h1 className="clients-list-page__title">Hotspot Clients</h1>
        <button 
