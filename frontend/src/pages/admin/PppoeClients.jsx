...
const PppoeClients = () => {
  ...
  useEffect(() => {
    ...
    fetch('/api/clients')
      ...
      .then(data => {
        setClients(data);
        setFilteredClients(data.filter(client => client.type === 'pppoe'));
      })
  ...
  const handleAddClient = (newClientData) => {
    ...
    setFilteredClients(newClients.filter(client => client.type === 'pppoe'));
  ...
  return (
    <div className="clients-list-page">
      <div className="clients-list-page__header">
        <h1 className="clients-list-page__title">PPPoE Clients</h1>
        <button 
